...
func (r *Repo) Get(id string) (domain.Connection, bool) { r.mu.RLock(); c, ok := r.connections[id]; r.mu.RUnlock(); return c, ok }
func (r *Repo) SetOnline(id string, v bool) { r.mu.Lock(); c := r.connections[id]; c.Online = v; r.connections[id] = c; r.mu.Unlock() }
