...
func (r *ProductRepo) Create(p domain.Product) domain.Product {
	r.s.mu.Lock()
	r.s.Products[p.ID] = p
	if _, ok := r.s.Models[p.ID]; !ok {
		r.s.Models[p.ID] = map[string]domain.ThingModel{}
	}
	r.s.mu.Unlock()
	return p
}
...
func (r *ProductRepo) SaveModel(productID, version string, m domain.ThingModel) domain.ThingModel {
	r.s.mu.Lock()
	if _, ok := r.s.Models[productID]; !ok {
		r.s.Models[productID] = map[string]domain.ThingModel{}
	}
