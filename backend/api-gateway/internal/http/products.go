...
func (h *ProductHandler) Create(w http.ResponseWriter, r *http.Request) {
    ...
    if err := json.NewDecoder(r.Body).Decode(&p); err != nil { w.WriteHeader(http.StatusBadRequest); json.NewEncoder(w).Encode(map[string]string{"error":"invalid body"}); return }
    if p.ID == "" { w.WriteHeader(http.StatusBadRequest); json.NewEncoder(w).Encode(map[string]string{"error":"id required"}); return }
    res := h.svc.Create(p)
