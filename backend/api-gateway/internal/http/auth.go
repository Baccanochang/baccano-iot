...
func Login(w http.ResponseWriter, r *http.Request) {
    ...
    if body.Username == "" || body.Password == "" { w.WriteHeader(http.StatusBadRequest); json.NewEncoder(w).Encode(map[string]string{"error":"invalid credentials"}); return }
    secret := os.Getenv("JWT_SECRET"); if secret == "" { secret = "dev-secret" }
...
func Refresh(w http.ResponseWriter, r *http.Request) {
    ...
    if err != nil { w.WriteHeader(http.StatusUnauthorized); json.NewEncoder(w).Encode(map[string]string{"error":"invalid token"}); return }
