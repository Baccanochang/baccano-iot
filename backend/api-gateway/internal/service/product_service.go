...
func (s *ProductService) SaveModel(productID, version string, m domain.ThingModel) domain.ThingModel { return s.repo.SaveModel(productID, version, m) }
...
func (s *ProductService) DiffModels(a, b domain.ThingModel) DiffResult {
    ...
    for _, p := range a.Properties { if id, ok := p["identifier"].(string); ok { amap[id] = struct{}{} } }
    for _, p := range b.Properties { if id, ok := p["identifier"].(string); ok { bmap[id] = struct{}{} } }
    var added, removed []string
    for k := range bmap { if _, ok := amap[k]; !ok { added = append(added, k) } }
    for k := range amap { if _, ok := bmap[k]; !ok { removed = append(removed, k) } }
